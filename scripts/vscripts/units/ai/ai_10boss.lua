function Spawn( entityKeyValues )
	...
	SmashAbility = thisEntity:FindAbilityByName( "my_fisure" )
	JumpAbility = thisEntity:FindAbilityByName( "ogre_tank_jump_smash" )
	LavaAbility = thisEntity:FindAbilityByName( "my_lava_boom" )
...
function OgreTankThink()
	...
	if GameRules:IsGamePaused() == true or thisEntity.castult == true then
		return 1
	end
    ...
    if LavaAbility ~= nil and LavaAbility:IsFullyCastable() then
		return Lava()
	end
...
function Lava()
	ExecuteOrderFromTable({
		UnitIndex = thisEntity:entindex(),
		OrderType = DOTA_UNIT_ORDER_CAST_NO_TARGET,
		AbilityIndex = LavaAbility:entindex(),
		Queue = false,
	})
