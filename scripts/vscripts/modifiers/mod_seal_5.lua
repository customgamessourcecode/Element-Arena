LinkLuaModifier( "modifier_my_relic_armor", "modifiers/mod_seal_5", LUA_MODIFIER_MOTION_NONE )
mod_seal_5 = class({})
--------------------------------------------------------------------------------
col_ud = 0

function mod_seal_5:GetEffectName()
	return "particles/my_doom_bringer_doom_ring.vpcf"
end

function mod_seal_5:IsHidden() 
	return true
end

--------------------------------------------------------------------------------

function mod_seal_5:IsPurgable()
	return false
end

----------------------------------------

function mod_seal_5:OnCreated( kv )
...
function mod_seal_5:DeclareFunctions()
	local funcs = 
	{
...
function mod_seal_5:OnWaweChange( wawe )
    self:SetStackCount(wawe)
end

function mod_seal_5:GetModifierPreAttack_BonusDamage( params )
	--if IsServer() then
...
function mod_seal_5:GetModifierPhysicalArmorBonus( params )
	--if IsServer() then
...
function mod_seal_5:GetModifierMagicalResistanceBonus( params )
	--if IsServer() then
...
function mod_seal_5:GetModifierAttackSpeedBonus_Constant( params )
	--if IsServer() then
...
function mod_seal_5:GetModifierBonusStats_Strength( params )
	--if IsServer() then
        local damag5 = self.bonus_all_stats_for_wawe * self:GetStackCount()
        return damag5
    --end
end

function mod_seal_5:GetModifierBonusStats_Agility( params )
	--if IsServer() then
        local damag6 = self.bonus_all_stats_for_wawe * self:GetStackCount()
        return damag6
    --end
end

function mod_seal_5:GetModifierBonusStats_Intellect( params )
	--if IsServer() then
        local damag7 = self.bonus_all_stats_for_wawe * self:GetStackCount()
        return damag7
    --end
end

function mod_seal_5:OnAbilityExecuted( params )
	if IsServer() then
...
function mod_seal_5:OnAttackLanded( params )
	if IsServer() then
...
function mod_seal_5:OnTakeDamage( params )
	if IsServer() then
