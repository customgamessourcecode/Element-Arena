...
function modifier_temple_guardian_wrath_thinker:OnCreated( kv )
	if IsServer() then
		...
		local nFXIndex = ParticleManager:CreateParticle( "particles/my_pugna_netherblast_pre.vpcf", PATTACH_CUSTOMORIGIN, nil )
...
function modifier_temple_guardian_wrath_thinker:OnIntervalThink()
	if IsServer() then
		local nFXIndex = ParticleManager:CreateParticle( "particles/my_pugna_netherblast.vpcf", PATTACH_CUSTOMORIGIN, nil )
		ParticleManager:SetParticleControl( nFXIndex, 0, self:GetParent():GetOrigin() )
