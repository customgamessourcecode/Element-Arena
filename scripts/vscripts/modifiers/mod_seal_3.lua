LinkLuaModifier( "modifier_my_relic_armor", "modifiers/mod_seal_3", LUA_MODIFIER_MOTION_NONE )
mod_seal_3 = class({})
--------------------------------------------------------------------------------
col_ud = 0

function mod_seal_3:GetEffectName()
	return "particles/my_doom_bringer_doom_ring.vpcf"
end

function mod_seal_3:IsHidden() 
	return true
end

--------------------------------------------------------------------------------

function mod_seal_3:IsPurgable()
	return false
end

----------------------------------------

function mod_seal_3:OnCreated( kv )
...
function mod_seal_3:DeclareFunctions()
	local funcs = 
	{
...
function mod_seal_3:OnWaweChange( wawe )
    self:SetStackCount(wawe)
end

function mod_seal_3:GetModifierPreAttack_BonusDamage( params )
	--if IsServer() then
...
function mod_seal_3:GetModifierPhysicalArmorBonus( params )
	--if IsServer() then
...
function mod_seal_3:GetModifierMagicalResistanceBonus( params )
	--if IsServer() then
...
function mod_seal_3:OnAbilityExecuted( params )
	if IsServer() then
...
function mod_seal_3:OnAttackLanded( params )
	if IsServer() then
...
function mod_seal_3:OnTakeDamage( params )
	if IsServer() then
